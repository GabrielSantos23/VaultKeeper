...
# Adjusted path for core/gdrive.py (parent=core, parent.parent=VaultKeeper)
...
class GoogleDriveManager:
    ...
    def upload_vault(self, vault_path: Path = None, progress_callback: Optional[Callable[[int], None]] = None) -> bool:
        ...
        try:
            ...
            if file_id:
                print(f"[GDrive] Updating existing file: {file_id}")
                url = f"{GOOGLE_UPLOAD_API}/files/{file_id}?uploadType=multipart"
                method = requests.patch
            else:
                print(f"[GDrive] Creating new file")
                url = f"{GOOGLE_UPLOAD_API}/files?uploadType=multipart"
                metadata["parents"] = [folder_id]
                method = requests.post
                
            files = {
                'data': ('metadata', json.dumps(metadata), 'application/json; charset=UTF-8'),
                'file': (vault_path.name, open(vault_path, 'rb'), "application/x-sqlite3")
            }
            
            response = method(url, headers=headers, files=files)
            response.raise_for_status()
            print("[GDrive] Upload complete")
            
            GoogleDriveManager._notify_sync_end(True)
            return True
            
        except Exception as e:
            print(f"[GDrive] Upload failed: {e}")
            GoogleDriveManager._notify_sync_end(False, str(e))
            raise e
        finally:
            self._is_syncing = False
