#!/usr/bin/env python3
"""
VaultKeeper - Native Messaging Host
Handles communication between the browser extension and the desktop app
"""
...
class NativeMessagingHost:
    ...
    def read_message(self) -> Optional[Dict[str, Any]]:
        ...
        Messages are prefixed with 4 bytes indicating the length.
        
        Returns:
            The parsed JSON message or None if failed
        """
        try:
            ...
            # Unpack as native byte order unsigned int
    ...
    def send_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            # Pack length as native byte order unsigned int
    ...
    def _handle_save_credentials(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """Handle save_credentials action."""
        if not self.auth.is_unlocked:
            return {'success': False, 'error': 'Vault is locked', 'locked': True}
    ...
    def _handle_delete_credentials(self, message: Dict[str, Any]) -> Dict[str, Any]:
        """Handle delete_credentials action."""
        if not self.auth.is_unlocked:
            return {'success': False, 'error': 'Vault is locked', 'locked': True}
    ...
    def _handle_get_all_credentials(self) -> Dict[str, Any]:
        """Handle get_all_credentials action."""
        if not self.auth.is_unlocked:
            return {'success': False, 'error': 'Vault is locked', 'locked': True}
    ...
    def run(self):
        ...
        while True:
            ...
            # Include _requestId in response for proper matching
